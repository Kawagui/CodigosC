...
/**								                                    **/
/**   Luís Guilherme Hitoshi Kanazawa		  14747868   02         **/
/**								                                    **/
/*********************************************************************/
...
#define false 0     // 0 equivale a falso  
#define true 1      // 1 equivale a verdadeiro
#define LETRAS 26   // Quantidade de letras do alfabeto
#define VALOR_a 97  // 97 em decimal equivale a 'a' na tabela ASCII
typedef int bool;   // Tipo bool equivale a tipo int
...
A estrutura NO foi projetada para armazenar os nos de nossa trie. Ela e composta por dois campos:
  ...
  - filhos: campo do tipo ponteiro para ponteiros de NOs
...
/*
Funcao que recebe como parametro o endereco da raiz de uma trie e realiza a inicializacao da estrutura. 
Isto e, atribui o valor 0 (zero) para o campo contador e o valor NULL para o campo filhos. */
...
/*
Funcao que aloca memoria para um novo no, inicializa seus campos (valor zero para contador e NULL para filhos)
e retorna o endereco do novo no.
*/
PONT criarNo(){
...
/*
Funcao recursiva que recebe o endereco de um no e retorna o numero de arranjos de ponteiros para filhos da trie a partir do no atual,
ou seja, retorna a quantidade de nos internos (nos que nao sao folha) da trie.
Se a funcao for chamada a partir da raiz, retornara o numero total de arranjos na trie.
*/
...
Palavras diferentes significa que cada palavra sera contada uma unica vez.
Se a funcao for chamada a partir da raiz, retornara o numero total de palavras diferentes na trie.
*/
...
Uma palavra sera contada mais de uma vez, caso o campo contador seja maior do que 1.
Se a funcao for chamada a partir da raiz, retornara o numero total de palavras na trie.
*/
...
/*
Funcao recursiva que recebe o endereco de um no, o endereco de um arranjo de caracteres e a posicao da letra atual da palavra.
Caso o no atual corresponda a letra final de uma palavra (contador maior que zero) imprime a palavra atual.
Se o no atual possuir filhos, realiza uma chamada recursiva para cada um de seus filhos
...
/*
Funcao que recebe o endereco da raiz de uma trie e o endereco de um arranjo de caracteres e imprime na tela, em ordem alfabetica,
todas as palavras da trie (utilizando a funcao exibirAux). O arranjo de caracteres, chamado palavra,
e usado para compor cada palavra a partir da raiz e entao imprimir essas palavras.
*/
...
Provavelmente voce desejara realizar a busca chamando uma funcao auxiliar recursiva (desenvolvida por voce) que tenha, ao menos,
um parametro adicional para indicar qual a letra atual da busca. Caso a palavra nao exista na trie,
sua funcao devera retornar 0 (zero), caso contrario,
...
/*
Funcao que recebe o endereco do no raiz de uma trie (raiz), o endereco de um arranjo de caracteres (palavra)
e o tamanho da palavra presente no arranjo de caracteres (n) e insere essa palavra na trie.
Provavelmente voce desejara realizar a insercao chamando uma funcao auxiliar recursiva (desenvolvida por voce) que tenha, ao menos,
um parametro adicional para indicar qual a letra atual da chamada recursiva
(assumindo que a cada chamada recursiva uma letra sera inserida/processada).
Voce pode assumir que todos os parametros desta funcao terao valores validos e a palavra a ser inserida
(presente no arranjo de caracteres) possuira apenas letras minusculas (sem acentos ou caracteres especiais).
A insercao funciona da seguinte forma (potencialmente recursiva):
    (i) se ainda falta inserir caractere(s) e o no atual nao possui arranjo de filhos, e necessario:
        ...
        (b) criar o no correspondente ao proximo caractere, acertar seus campos 
            (se voce usar a funcao criarNo ela ja acerta esses valores)
            e colocar o endereco dele na posicao correspondente do arranjo filhos; e
        (c) continuar recursivamente com o processo sendo que, se voce acabou de criar o ultimo caractere da palavra,
            o campo contador deve receber o valor 1 e a insercao foi concluida.
    (ii) se ainda falta inserir caractere(s) e o no atual possui o arranjo de filhos:
        caso o no correspondente ao proximo caractere ja exista no arranjo de filhos, basta prosseguir a insercao (recursivamente)
        a partir dele, caso contrario e necessario prosseguir de acordo com o processo (i.b).
    (iii) se voce chegou ao ultimo caractere da palavra e ele ja existe na trie,
...
/*
Funcao que recebe o endereco do no raiz de uma trie (raiz), o endereco de um arranjo de caracteres (palavra)
    e o tamanho da palavra presente no arranjo de caracteres (n) e exclui todas as copias dessa palavra da trie.
    Provavelmente voce desejara realizar a exclusao chamando uma funcao auxiliar recursiva (desenvolvida por voce) que tenha,
    ao menos, um parametro adicional para indicar qual a letra atual da chamada recursiva 
    (assumindo que cada chamada recursiva avancara uma letra na arvore).
    Adicionalmente, se a operacao de exclusao resultar na eliminacao de um no, pode ser necessario, recursivamente,
    apagar alguns dos nos anteriores, neste caso, voce pode precisar de mais um parametro adicional na sua funcao auxiliar recursiva
    ou ela pode ter um retorno (potencialmente booleano) para indicar que e necessario excluir nos na volta da chamada recursiva.
    Voce pode assumir que todos os parametros desta funcao terao valores validos e a palavra a ser excluida
    (presente no arranjo de caracteres) possuira apenas letras minusculas (sem acentos ou caracteres especiais).
    E possivel que a palavra nao exista na sua trie, entao a funcao de exclusao nao causara nenhuma mudanca na trie.
    A exclusao funciona da seguinte forma (potencialmente recursiva):
Voce navegara na trie, caractere a caractere, ate passar por toda a palavra, isto e, chegar no caractere final da palavra. 
    (i) Se isto nao for possivel, isto e, se a sequencia de caracteres que forma a palavra nao esta presente na trie,
        sua funcao devera encerrar sem realizar nenhuma mudanca na estrutura; caso contrario, ha duas possibilidades principais:
    (ii) A palavra nao existe na trie (isto e, apesar de suas letras estarem la, o campo contador da ultima letra e igual a zero),
        neste caso, sua funcao deve encerrar sem modificar a estrutura;    
    (iii) A palavra existe na trie, neste caso ela dever ser excluida, considerando duas situacoes:
        (a) O no correspondente a ultima letra possui filhos, neste caso o contador dele devera ser zerado
            e nao ha mais nada a ser feito pela funcao; 
        (b) Caso o no correspondente a ultima letra nao possua filhos ele devera ser excluido,
            e o ponteiro para ele no arranjo de filhos de seu pai deve ser atualizado para NULL.
            Se apos essa exclusao, o pai desse no nao possuir mais filhos, seu arranjo de filhos deve ser excluido (memoria liberada)
            e seu campo filhos deve receber o valor NULL. Adicionalmente, se o campo contador do no pai valer zero
            este tambem deve ser apagado (e o processo iii.b deve ser repetido enquanto cada no [na volta da recursao]
            nao possuir mais filhos e nao for um no final de uma palavra).
Observacao: o no raiz nunca devera ser excluido, porem seu arranjo de filhos podera ser excluido caso este no nao possua filhos
...
/*
Funcao que recebe o endereco do no raiz de uma trie (raiz), o endereco de um arranjo de caracteres (palavra)
e o tamanho da palavra presente no arranjo de caracteres (n) e exclui uma copia dessa palavra da trie
(isto e, caso a palavra exista na trie, diminui o contador correspondente a sua ultima letra em uma unidade).
Observacoes: se a palavra nao existir na trie, nao ha nada a ser feito pela funcao; se a palavra existir
